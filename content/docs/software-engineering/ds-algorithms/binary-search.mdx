...
A way to to take a big problem and cut it in half over and over again until we've reached an answer.
...
Given a range of numbers from 1 to 80. You have to guess a number in the least amount of moves. Each time you guess, you're told if its too high or too low. The first guess would be 40 because its in the middle and it automatically eliminates half the answers. Based on this answer, you make another guess and cut the range in half again, and again, and so on until you find your answer. Thats binary search. Each guess eliminates half the answers. This gives us `O(logN)` time.

### Monotonic condition

You don't necessarily need a sorted array for binary search to work. All we really need is a `Monotonic Condition`,

a `Monotonic Condition` only ever changes in one direction. If its increasing, it will never decrease. If its dereasing it won't ever increase.
...
- sorted array
- function that returns a boolean and transitions from returning False to only ever returning True. Binary search could help us find the first occurence of True in `O(logN)` time as long as we can write a feasible function that can aswer if an index is valid.
  - comes up a lot on problems where we are not given a sorted array but are trying to min/max something and we can check if a solution if valid for a given guess

```python
# vanilla implementation
def binary_search(arr: List[int], target: int) -> int:
    # left start at beginning of list, right at the end of list
    ...
    while(left <= right):
        ...
        # so we discard the right half, including mid, and move our right pointer, mid -1 1
        else:
            right = mid - 1

    # if we reach here, we've search the entire list and did not find the target
    return -1
```
...
def binary_search(arr: List[int], target: int) -> int:
    ...
    # but we keep searching left to see if there is an even earlier occurence
    first_true_index = -1
    ...
    while(left <= right):
        ...
        # we check if this mid point value satisfies some condition
        # we are asking "is this index good enough?"
        if feasible(mid):
            # this mid is "good enough" and we store it as a possible answer
            # and then move left in-case their is an earlier index that also works
